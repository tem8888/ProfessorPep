...
module.exports = async (client, msg) => {
 ...
 let txt_url = ''
    ...
    if ((typeof quizLine !== 'undefined') && questionInProgress) { // Существует ли такая переменная QuizLine?
        check = stringSimilarity.compareTwoStrings(msg.content.toLowerCase(), answer.toLowerCase()); 
    }
    ...
    if ((check == 1) && (firstAnswer)) {
        ...
        if (qNumber) { 
            ...
            if (k >= qNumber) return GetTop.top(msg).then(()=>{ // если это был последний вопрос турнира, то возвращаем итоговую таблицу
            ...
            setTimeout(async function(){
                ...
                next.run(client, msg, question, answer, qNumber, k).then( async(hintArray) => {
                    await GetHints.interval(msg, hintArray, qNumber).then( async() => { // Срабатывает только когда на вопрос не был дан правильный ответ
        ...
        setTimeout(async function(){
            ...
            next.run(client, msg, question, answer).then( async(hintArray) => {
                await GetHints.interval(msg, hintArray).then( async () => { // Срабатывает только когда на вопрос не был дан правильный ответ
                    msg.channel.send(`${congrat.noAnswer[`${randCongrat}`]} **${answer}**`);
    ...
    GuildModel.findOne({idGuild: msg.guild.id}).then( async (guild) => {
        ...
        if (command.startsWith(prefix)) {
            ...
            if (commandfile) commandfile.run(client, msg, args).then((qNum)=>{
...
/*
	    ...
	    if ((msg.content === `${prefix}старт`) && (!questionInProgress)) {
            ...
            GetQuizLine.getLine(); // Из answer.js получаю номер строки, вопрос и ответ
  	        ...
  	        next.run(client, msg, question, answer, qNumber, k).then( async (hintArray) => {
  	    	    await GetHints.interval(msg, hintArray, qNumber).then( async (qcount) => { // Срабатывает только когда на вопрос не был дан правильный ответ
                    

  	    		    if (!qNumber) { // если турнира нет и ответ не угадан
                        msg.channel.send(`${congrat.noAnswer[`${randCongrat}`]} **${answer}**`);
                        ...
                        k += qcount;
                        msg.channel.send(`${congrat.noAnswer[`${randCongrat}`]} **${answer}**`);
                        if (k < qNumber) { // если это не последний вопрос в турнире
    ...
    if (msg.content.startsWith(`${prefix}upl`)) {
       ...
       // msg.delete();
        ...
        if (txt_url !== '' && txt_url.slice(-3) === 'txt')
            ...
            msg.channel.send(`Загружен список из **${fullData.length}** вопросов`);
        })
    }
    if (msg.content.startsWith(`${prefix}default`)) {
        fullData = fs.readFileSync(path.join(__dirname, '../new-file.txt'),'utf8').split('\n');
        msg.channel.send(`Загружен дефолтный список из **${fullData.length}** вопросов`);
    }
